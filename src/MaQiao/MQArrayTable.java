...
public class MQArrayTable {
	...
	public static class TableClass {
		 ...
		 * @return int
		 */
		int getLenColumnMax(int column) {
			int lenMax = 0;
			int columnMax = getUnitCountMax();
		 ...
		 * @param columns int[]
		 */
		void setAlign(Align align, int... columns) {
			for (int i = 0, len = rowList.size(); i < len; i++) {
				LineClass e = rowList.get(i);
		 ...
		 * @param location int[]
		 */
		void setAlignColumnsLocation(Align align, int... location) {
			/* 成对出现，用于定位表的位置 */
